        ...
        for item1, item2 in field.choices:
            if isinstance(item2, (list, tuple)):
                ...
                for inner_val, inner_label in group_items:
                    selected = (True if field.data and inner_val in field.data
                                else False)
                    html.append(
                        self.render_option(inner_val, inner_label, selected))
                html.append('</optgroup>')
            else:
                selected = True if field.data and val in field.data else False
                val = item1
                label = item2
